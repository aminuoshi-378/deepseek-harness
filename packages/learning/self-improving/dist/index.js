...
class ExperienceStore {
    ...
    store(sessionId, turnId, outcomeScore, userFeedback, toolsUsed, actions, workspaceDigest) {
        const id = ulid();
    ...
    count() {
        return this.db.prepare('SELECT COUNT(*) as c FROM experiences').get().c;
    }
    stats() {
    ...
    similarity(rec, tools, ws) {
        ...
        if (ws && rec.workspaceDigest && rec.workspaceDigest === ws) {
            weight += 0.4;
            score += 0.4;
        }
    ...
    close() { this.db.close(); }
    clear() { this.db.exec('DELETE FROM experiences'); }
}
// ---------------------------------------------------------------------------
// Plugin apply — wires everything to dsh's real event hooks
// ---------------------------------------------------------------------------
const PLUGIN_SOURCE = { kind: 'plugin', plugin: 'self-improving' };
    ...
    ctx.on('agent/turn-stopping', async (payload) => {
        ...
        if (goalProgress === 'none') {
            try {
                ...
                const turnEndEvents = events.filter((e) => e.type === 'turn/end' && e.data?.turn === turn);
                ...
                if (lastTurnEnd) {
                    ...
                    if (reason?.kind === 'completed')
                        ...
                        goalProgress = 'advanced'; // unknown → assume advanced
                }
                else {
        ...
        try {
            const events = agent.session.events ?? [];
            guardCount = events.filter((e) => e.type === 'user/message' &&
                e.data?.source?.plugin === 'repeat-tool-reminder').length;
        }
        ...
        if (feedbackService && typeof feedbackService.list === 'function') {
            try {
                ...
                if (Array.isArray(items) && items.length > 0) {
                    const turnAssistantSeqs = new Set();
                    // Find assistant messages from this turn
                    const events = agent.session.events ?? [];
                    for (const e of events) {
                        if (e.type === 'assistant/message' && e.data?.turn === turn) {
                            turnAssistantSeqs.add(e.seq);
                        }
                    }
                    // Check if any feedback targets messages from this turn
                    const turnFeedback = items.filter((item) => turnAssistantSeqs.has(item.messageSeq) ||
                        turnAssistantSeqs.has(item.messageId));
                    if (turnFeedback.length > 0) {
                        const hasNegative = turnFeedback.some((f) => f.rating === 'negative');
                        const hasPositive = turnFeedback.some((f) => f.rating === 'positive');
        ...
        const outcomeScore = Math.max(0, Math.min(1, goalScore * 0.4 + toolSuccessRate * 0.25 + (0.15 - guardPenalty) + feedbackScore * 0.2));
        ...
        const expId = store.store(agent.id, `turn-${turn}`, outcomeScore, userFeedback, toolsUsed, actions, wsDigest);
        ...
        ctx.on('agent/pre-step', async (payload, next) => {
            ...
            try {
                ...
                if (records.length > 0) {
                    ...
                    const lines = ['## Past Experience (advisory)', ''];
                ...
                log(`pre-step injection error: ${err.message}`);
                return next();
            }
        });
    }
    ...
    if (config.behaviorAdapterEnabled) {
        ctx.systemPrompt.section({
            ...
            text: () => {
                ...
                if (stats.total < 10)
                    return '';
                const lines = ['## Learned Preferences (advisory)', ''];
